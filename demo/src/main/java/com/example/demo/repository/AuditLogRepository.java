...
public interface AuditLogRepository extends JpaRepository<AuditLog, Long> {
    ...
    // Obtener logs relevantes para el dashboard (logins de admin y CRUD de ABM)
    @Query("SELECT a FROM AuditLog a LEFT JOIN FETCH a.usuario u LEFT JOIN FETCH a.rol r " +
            "WHERE (a.accion = 'INICIO_SESION' AND r.nombre = 'ADMIN') OR " +
            "a.accion IN ('ALTA_USUARIO', 'MODIFICACION_USUARIO', 'BAJA_USUARIO', 'REACTIVAR_USUARIO', " +
            "'CREAR_OFERTA', 'MODIFICAR_OFERTA', 'ELIMINAR_OFERTA', 'CAMBIAR_ESTADO_OFERTA') " +
            "ORDER BY a.fecha DESC, a.hora DESC")
    Page<AuditLog> findDashboardRelevantLogs(Pageable pageable);
}
